...
from oricswin.models.cswin import CSWin_64_24322_small_224
import timm
...
def load_pretrained_weights(img_size, model_scale):
    ...
    if (model_scale == 'tiny'):
        if img_size == 224:
            ...
            print('Loading:',
                  './pretrained_pth/maxvit/maxvit_tiny_rw_224_sw-7d0dffeb.pth')
            state_dict = torch.load(
                './pretrained_pth/maxvit/maxvit_tiny_rw_224_sw-7d0dffeb.pth')
        elif (img_size == 256):
            backbone = maxvit_rmlp_tiny_rw_256_4out()
            print(
                'Loading:', './pretrained_pth/maxvit/maxvit_rmlp_tiny_rw_256_sw-bbef0ff5.pth')
            state_dict = torch.load(
                './pretrained_pth/maxvit/maxvit_rmlp_tiny_rw_256_sw-bbef0ff5.pth')
        else:
            sys.exit(str(
                img_size)+" is not a valid image size! Currently supported image sizes are 224 and 256.")
    elif (model_scale == 'small'):
        if img_size == 224:
            backbone = maxvit_rmlp_small_rw_224_4out()  # [64, 128, 320, 512]
            print(
                'Loading:', './pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
            state_dict = torch.load(
                './pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
        elif (img_size == 256):
            backbone = maxxvit_rmlp_small_rw_256_4out()
            print(
                'Loading:', './pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
            state_dict = torch.load(
                '/home/dc/hdd1/vscode/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
        else:
            sys.exit(str(
                img_size)+" is not a valid image size! Currently supported image sizes are 224 and 256.")
    else:
        sys.exit(
            model_scale+" is not a valid model scale! Currently supported model scales are 'tiny' and 'small'.")
    ...
    def __init__(self, n_class=1, img_size=224):
        ...
        if img_size == 224:
            ...
            print(
                'Loading:', './pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
            state_dict = torch.load(
                './pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
        else:
            sys.exit(str(
                img_size)+" is not a valid image size! Currently supported image sizes are 224 and 256.")
...
class MaxViT_CASCADE(nn.Module):
    def __init__(self, n_class=1, img_size=224, model_scale='small', decoder_aggregation='additive'):
        ...
        if (decoder_aggregation == 'additive'):
            ...
            sys.exit("'"+decoder_aggregation +
                     "' is not a valid decoder aggregation! Currently supported aggregations are 'additive' and 'concatenation'.")
...
class MERIT_Parallel(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear'):
        ...
        # backbone network initialization with pretrained weight
        self.backbone1 = load_pretrained_weights(
            self.img_size_s1[0], self.model_scale)
        self.backbone2 = load_pretrained_weights(
            self.img_size_s2[0], self.model_scale)
        ...
        if (self.decoder_aggregation == 'additive'):
            ...
            sys.exit("'"+self.decoder_aggregation +
                     "' is not a valid decoder aggregation! Currently supported aggregations are 'additive' and 'concatenation'.")
    ...
    def forward(self, x):
        ...
        # transformer backbone as encoder
        f1 = self.backbone1(F.interpolate(
            x, size=self.img_size_s1, mode=self.interpolation))
        # print([f1[3].shape,f1[2].shape,f1[1].shape,f1[0].shape])

        f2 = self.backbone2(F.interpolate(
            x, size=self.img_size_s2, mode=self.interpolation))
        ...
        p21 = F.interpolate(p21, size=(
            p11.shape[-2:]), mode=self.interpolation)
        p22 = F.interpolate(p22, size=(
            p12.shape[-2:]), mode=self.interpolation)
        p23 = F.interpolate(p23, size=(
            p13.shape[-2:]), mode=self.interpolation)
        p24 = F.interpolate(p24, size=(
            p14.shape[-2:]), mode=self.interpolation)
...
def load_checkpoint(args, model, checkpoint_path, use_ema=False):
    ...
    unexpect_dic = [k for k, v in model_dict.items()
                    if not (k in pretrained_dict)]
    if True:
        print("Miss Keys:", len(miss_dic))
        print("Ubexpected Keys:", len(unexpect_dic))
...
class MERIT_Cascaded(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear'):
        ...
        self.backbone1 = load_pretrained_weights(
            self.img_size_s1[0], self.model_scale)
        self.backbone2 = CSWin_64_24322_small_224()
        load_checkpoint(None, self.backbone2, "/home/dc/hdd1/vscode/cswin_small_224.pth", True)
        
        self.b2_c_up=nn.ModuleList()
        self.b2_c_up.append(nn.Conv2d(64,96,kernel_size=7,padding=3))
        self.b2_c_up.append(nn.Conv2d(128,192,kernel_size=5,padding=2))
        self.b2_c_up.append(nn.Conv2d(256,384,kernel_size=3,padding=1))
        self.b2_c_up.append(nn.Conv2d(512,768,kernel_size=1))
    ...
    def forward(self, x):
        ...
        if (x.shape[2] % 14 != 0):
            ...
            f1 = self.backbone2(F.interpolate(
                x, size=self.img_size_s1, mode=self.interpolation))
        ...
        if (x.shape[2] % 14 != 0):
            f2 = self.backbone2(F.interpolate(
                x_in, size=self.img_size_s2, mode=self.interpolation))
        else:
            f2 = self.backbone1(F.interpolate(
                x_in, size=self.img_size_s2, mode=self.interpolation))
        for i in range(0, len(f2)):
            
            B, L, C = f2[i].shape
            h = w = int(math.sqrt(L))
            f2[i] = rearrange(f2[i], "b (h w) c -> b c h w", h=h, w=w)
            f2[i]=self.b2_c_up[i](f2[i])
            # print(f2[i].shape)

        skip1_0 = F.interpolate(f1[0], size=(
            f2[0].shape[-2:]), mode=self.interpolation)
        skip1_1 = F.interpolate(f1[1], size=(
            f2[1].shape[-2:]), mode=self.interpolation)
        skip1_2 = F.interpolate(f1[2], size=(
            f2[2].shape[-2:]), mode=self.interpolation)
        skip1_3 = F.interpolate(f1[3], size=(
            f2[3].shape[-2:]), mode=self.interpolation)

        x21_o, x22_o, x23_o, x24_o = self.decoder(
            f2[3]+skip1_3, [f2[2]+skip1_2, f2[1]+skip1_1, f2[0]+skip1_0])
...
class MERIT_Parallel_Small(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), decoder_aggregation='additive', interpolation='bilinear'):
        ...
        # [64, 128, 320, 512]
        self.backbone1 = maxxvit_rmlp_small_rw_256_4out()
    ...
    def forward(self, x, im1_size=(256, 256), im2_size=(224, 224)):
        ...
        if (x.shape[2] % 14 != 0):
            f1 = self.backbone1(F.interpolate(
                x, size=im1_size, mode=self.interpolation))
            f2 = self.backbone2(F.interpolate(
                x, size=im2_size, mode=self.interpolation))
        else:
            f1 = self.backbone2(F.interpolate(
                x, size=im1_size, mode=self.interpolation))
            f2 = self.backbone1(F.interpolate(
...
class MERIT_Cascaded_Small(nn.Module):
    ...
    def forward(self, x, im1_size=(256, 256), im2_size=(224, 224)):
        ...
        if (x.shape[2] % 14 != 0):
            f2 = self.backbone2(F.interpolate(
                x_in, size=im2_size, mode=self.interpolation))
        else:
            f2 = self.backbone1(F.interpolate(
                x_in, size=im2_size, mode=self.interpolation))
