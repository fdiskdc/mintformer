...
class Attention_block(nn.Module):
    def __init__(self,F_g,F_l,F_int,scale=(1, 3, 5, 7)):
        super(Attention_block,self).__init__()
        self.agg=PatchModify_transXNet((F_g+F_l), (F_g+F_l)*2, F_int,scale=scale)
    ...
    def forward(self,g,x):
        agg_out = torch.cat([g, x], 1)
        agg_out=self.agg(agg_out)
        
        psi = self.relu(agg_out)
        psi = self.psi(psi)
...
class CASCADE_Add(nn.Module):
    def __init__(self, channels=[512,320,128,64]):
        ...
        while i*2 <= 14:
            ...
            i+=2
        # print(scale)
        self.AG3 = Attention_block(F_g=channels[1],F_l=channels[1],F_int=channels[2],scale=scale)#16*16 14*14
        ...
        while i*2 <= 28:
            ...
            i+=2
        # print(scale)
        self.AG2 = Attention_block(F_g=channels[2],F_l=channels[2],F_int=channels[3],scale=scale)#32*32 28*28
        ...
        while i*2 <= 56:
            ...
            i+=2
        # print(scale)
        self.AG1 = Attention_block(F_g=channels[3],F_l=channels[3],F_int=int(channels[3]/2),scale=scale)#64*64 56*56
